...
async function dispatch(runtime: RVEngine, args: string, ctx: ExtensionCommandContext, ompVersion: string): Promise<void> {
  ...
  switch (sub) {
    ...
    default:
      ctx.ui.notify(`RV · unknown subcommand "${sub}". Try: setup, status, usage, doctor, review, on, off, config`, "warning");
  }
}
...
export function registerRvCommand(pi: ExtensionAPI, runtime: RVEngine): void {
  ...
  pi.registerCommand("rv", {
    description: "Resolve Vector — cross-model review (setup | status | usage | doctor | review | on [mode] | off | config)",
    getArgumentCompletions: (prefix) => {
      const subs = ["setup", "status", "usage", "doctor", "review", "on", "off", "config", "best", "fuse", "compare"];
